...
The goal of the project is to create a simple, text-only clone of the game Wordle in Python - to be able to better understand its logic and how it works.  

The rules for the game are:  
  * Random 5 letter long target word selected for each iteration of the game  
...
The game should go through the following steps:  
  * When the program begins, show a help message – explaining syntax and input options - the help message should also be displayed if user inputs ‘help’ (case insensitive) at any time  
  * Get random target word  
  * Prompt for guess (case insensitive)  
  * Validate guess (length, in valid words)
  * If the guess is invalid – loop back to prompt with explanation message
  * If the guess is valid – score the guess (assigning correct letters and letter positions)  
...
The project should follow PEP8 standard, and should be clearly legible to anyone reading the code
